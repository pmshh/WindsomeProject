...
public class QaService {
    ...
    public boolean validatePost(Account account, Long qaId, String password) {
        // 관리자 권한 갖고 있을 시 바로 통과
        if (account.getState() == Role.ADMIN) {
            return true;
        }
    ...
    public List<QaDtlDto> getQaDtl(Long qaId) {
        return qaRepository.getQaDtl(qaId)
                .stream()
                .map(q -> new QaDtlDto(q, accountRepository.findById(q.getAccountId()).orElseThrow(EntityNotFoundException::new).getName()))
